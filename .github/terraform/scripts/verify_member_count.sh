...
verify_hazelcast_cluster_size() {
    EXPECTED_SIZE=$1
    for i in `seq 1 5`; do
        local MEMBER_COUNT=$( curl http://127.0.0.1:5701/hazelcast/health/cluster-size )

        if [ "$MEMBER_COUNT" == "$EXPECTED_SIZE" ] ; then            echo "Hazelcast cluster size equal to ${EXPECTED_SIZE}"
            return 0
        else
